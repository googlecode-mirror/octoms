<?php if (count($this->v('info'))>0):foreach ($this->v('info') AS $result):if (count($result)>3):?>

<?php $params = array(); foreach ($result AS $k=>$v):if (strpos($k, 'param')!==FALSE):$params[] = $v;endif;endforeach;?>
